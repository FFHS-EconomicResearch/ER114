...
###################################################
#downloaded: https://github.com/TaddyLab/MBAcourse/blob/master/examples/oj.R


library(tidyverse)
## read in the data
oj <- read_csv(xfun::from_root("data","raw","oj_(taddy_github).csv") )
head(oj)
...
oj %>% ggplot(aes(x=price,fill=brand)) +
          geom_boxplot() + scale_fill_manual(values=brandcol)
oj %>% ggplot(aes(x=log(price),fill=brand)) +
          geom_boxplot() + scale_fill_manual(values=brandcol)

oj %>% ggplot(aes(x=price,y=log(sales),color=brand)) +
          geom_point()+ scale_color_manual(values=brandcol)

oj %>% ggplot(aes(x=log(price),y=log(sales),color=brand)) +
  geom_point()+ scale_color_manual(values=brandcol)
